 ...
 *
 * @brief
 * This file implement the interface of eCompass algorithm adapter for 3+2.
 */
...
static COMPASSLIB_H5_INIT_STRUCT init_parm;

int ids_h5_nvm_restore()
{
...
uint16 ids_h5_nvm_read(uint16 offset)
{
	if (offset < CompassLib_H5_GetNVMBlockSize()) {
...
uint8 ids_h5_nvm_write(uint16 offset, uint8 data)
{
	if (offset < CompassLib_H5_GetNVMBlockSize()) {
		NVM[offset] = data;
		return 1;
...
{
	COMPASSLIB_H5_BUILD_DATE builddate;
	COMPASSLIB_H5_INIT_RESULT initRes;
	int i;
...
#if SENSORS_ALGO_DEBUG
	builddate = CompassLib_H5_GetBuildDate();
	ALOGD("Algorithm Library Info:\n");
	ALOGD("Build date: %02x-%02x-%02x, %02x:%02x:%02x\n",
		builddate.Year, builddate.Month, builddate.Day,
		builddate.Hour, builddate.Minute, builddate.Second);
	ALOGD("NVM block size: %d\n", CompassLib_H5_GetNVMBlockSize());
	ALOGD("Version: %04x\n", CompassLib_H5_GetVersion());
	ALOGD("Auto-Calibration: %d\n", CompassLib_H5_IsAutoCalibrationEnabled());
#endif

	init_parm.Size = sizeof(COMPASSLIB_H5_INIT_STRUCT);
	init_parm.VersionHi = COMPASSLIB_H5_VERSION_HI;
	init_parm.VersionLo = COMPASSLIB_H5_VERSION_LO;
	init_parm.NVM_Read = ids_h5_nvm_read;
	init_parm.NVM_Write = ids_h5_nvm_write;
	initRes = CompassLib_H5_Init(&init_parm);

	ALOGD("CompassLib init result: %d\n", initRes);
...
{
	CompassLib_H5_ClearCalibration();
	return 0;
}
...
{
	assert(d != NULL);
	CompassLib_H5_DataIn(d->gx, d->gy, d->hx, d->hy, d->hz);
}
...
int ids_h5_calc_orientation(struct SensorData_Orientation *o, struct SensorData_Algo *d)
{
	...
	COMPASSLIB_H5_ORIENTATION orientation;
	d->gz = -32768.0;
#if FILTER
	...
	noiseLevel = CompassLib_H5_GetMagneticNoiseLevel();
	Tquality = CompassLib_H5_GetCalibrationQualityFactor();
	...
	if (1){//((Tquality > 0) && (noiseLevel < 2)) {
		...
		od.gx += ((d->gx - od.gx) >> FILTER_LEVEL);
		od.gy += ((d->gy - od.gy) >> FILTER_LEVEL);
		od.gz += ((d->gz - od.gz) >> FILTER_LEVEL);
		od.hx += ((d->hx - od.hx) >> FILTER_LEVEL);
		od.hy += ((d->hy - od.hy) >> FILTER_LEVEL);
		od.hz += ((d->hz - od.hz) >> FILTER_LEVEL);
		memcpy(d, &od, sizeof(od));
	#endif
		orientation = CompassLib_H5_CalculateOrientation(d->gx, d->gy, d->hx, d->hy, d->hz);
...
{
	COMPASSLIB_H5_STATE state;
	struct timespec tv_req, tv_rem;
	int n;

	assert(d != NULL);

	switch (CompassLib_H5_GetOperatingState())
	{
	case COMPASSLIB_H5_STATE_UNAVAILABLE:
	case COMPASSLIB_H5_STATE_OFF:
		// compass library requires no CPU time
		break;
			
	case COMPASSLIB_H5_STATE_ON:
	case COMPASSLIB_H5_STATE_BUSY:
		...
		for (n = 0; n < 5; n++) {
			...
			state = CompassLib_H5_Task(ids_get_milliseconds());
			if (state != COMPASSLIB_H5_STATE_BUSY)
				break; // no more CPU time needed			
		}
	#else	/* make sure auto-calibration well done no matter how much CPU time consume */
		do {
			state = CompassLib_H5_Task(ids_get_milliseconds());
		} while ((state == COMPASSLIB_H5_STATE_BUSY));
	#endif
		break;
	default:
		break;
...
void dump(const float v[], int size, int type, const char *str)
{
	...
	ALOGD("Dump %s - %s:", str, type ? "MAX": "MIN");
        for (i = 0; i < size; i++) {
		ALOGD(" %1.3f", v[i]);
	}
	ALOGD("\n");
...
int ids_h5_calc_magcal_data(struct SensorData_Algo *d, float cald[3])
{
	...
	assert(d != NULL);
	Mag_Algo_Offset = CompassLib_H5_Get_Offset();
